...
String SoftAP_NAME PROGMEM = "BT_" + deviceID;
...
void setup(){
  ...
  if(initFS(LITTLEFS)){
    ...
    if(isFile(LITTLEFS,"/config.json")){
      ...
      //Set mqtt parameter
      pbPubTopic = pbPubTopic  + pb_merchantid +"/"+ pb_uuid;
      pbSubTopic = pbSubTopic + pb_merchantid +"/"+ pb_uuid;
...
void pbRegCallback(char* topic, byte* payload, unsigned int length){
  ...
  if(String(cfginfo.payboard.uuid).isEmpty()){
    if(!doc["uuid"].isNull()){
      pb_uuid = doc["uuid"].as<String>();
      cfginfo.payboard.uuid = pb_uuid.c_str();
      //cfginfo.payboard.uuid = doc["uuid"].as<String>().c_str();
      ...
      //*** Save uuid to eeprom.
      cfgdata.begin("config",false);
      cfgdata.putString("uuid",pb_uuid);
      cfgdata.end();
...
void pbRegisMqtt(){
  ...
  while(!mqclient.connect(deviceID.c_str(),cfginfo.payboard.mqttuser,cfginfo.payboard.mqttpass)){
    Serial.printf(".");
    delay(500);
  }
...
void pbBackendMqtt(){
          Serial.println("Checkpoint #3 in loop");
          Serial.println(pb_uuid);
          Serial.println(pb_merchantid);
          Serial.println(pb_mqtthost);
          Serial.println(pb_mqttport);
          Serial.println(pb_mqttuser);
          Serial.println(pb_mqttpass);

    if(!mqclient.connected()){
      mqclient.setServer(pb_mqtthost.c_str(),pb_mqttport);
      mqclient.setCallback(pbCallback);
      pbSubTopic = "payboard/" + pb_merchantid+ "/" + pb_uuid;
    
      Serial.printf("Mqtt connecting ...");
      while(!mqclient.connect(deviceID.c_str(),pb_mqttuser.c_str(),pb_mqttpass.c_str())){
        Serial.printf(".");
        delay(500);
      }
      Serial.printf("connected\n");
